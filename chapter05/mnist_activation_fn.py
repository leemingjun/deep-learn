...
def search_best_model_ex():
    ...
    for activation_fn in activation_fn_list:
        # 激活函数名称
        activation_func_name = getattr(activation_fn, '__name__')
        classifier = tf.estimator.DNNClassifier(
            feature_columns=feature_columns,
            ...
            dropout=0.1,
            model_dir=("./tmp/activation_{}/mnist_model".format(activation_func_name)))
        
        score = mnist_model(classifier, train_input_fn, test_input_fn)
        
        log_msg = "activation_fn={}  scores: {:.2f}%".format(activation_func_name, score)
        print (log_msg)

...
def search_best_model_ex():
    ...
    x = []
    y = []
    for dropout in np.arange(0.1, 0.9, 0.1):
        x.append(dropout)
        classifier = tf.estimator.DNNClassifier(
            feature_columns=feature_columns,
            ...
            optimizer=adam_optimizer,
            n_classes=10,
            # 测试不同dropout取值对模型准确率的影响
            dropout=dropout,
            model_dir=('./tmp/dropout{:d}/mnist_model'.format((10 * dropout).astype(np.int32))))
        
        score = mnist_model(classifier, train_input_fn, test_input_fn)
        y.append(score)
        
        print ("dropout={:.2f}  scores: {:.2f}%".format(dropout, score))
    
    plt.plot(x, y, label="准确率", linestyle='-', linewidth=1, color='black') 
    
    plt.xlabel("Dropout的取值")
    plt.ylabel("准确率")

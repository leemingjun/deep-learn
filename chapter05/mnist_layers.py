...
def read_mnist_data(data_type="train"):
    img_path = ('./data/mnist/%s-images-idx3-ubyte.gz' % data_type)
    label_path = ('./data/mnist/%s-labels-idx1-ubyte.gz' % data_type)
    ...
    if not os.path.exists(img_path) or not os.path.exists(label_path):
        ...
        img_url = (
            'http://yann.lecun.com/exdb/mnist/%s-images-idx3-ubyte.gz' % data_type)
        print("下载：%s" % img_url)
        ...
        # 从网上下载标签数据，并且保存到本地
        label_url = (
            'http://yann.lecun.com/exdb/mnist/%s-labels-idx1-ubyte.gz' % data_type)
...
# 分别比较隐藏层的层数为，2层、3层、4层、5层时，对模型识别准确率的影响


def model_layers_selection():
    features, labels = read_mnist_data(data_type="train")
    train_input_fn = tf.estimator.inputs.numpy_input_fn(
        x={"x": features},
        y=labels,
    ...
    # 保持神经元的总数不变，改变隐藏层的层数、以及每层的神经元的数量
    hidden_layers_list = [[500, 500], [400, 300, 300], [250, 250, 250, 250],
                          [200, 200, 200, 200, 200]]
    y = []
    for hidden_layers in hidden_layers_list:
        classifier = tf.estimator.DNNClassifier(
            ...
            n_classes=10,
            dropout=0.1,
            model_dir=('./tmp/layers_%d_%d/mnist_model' % (hidden_layers[0],
                                                           hidden_layers[1])),
            config=tf.estimator.RunConfig().replace(save_summary_steps=10))
    ...
    # 将比较结果可视化
    # 隐藏层神经元的层数，分别是2层、3层、4层、5层
    x = [2, 3, 4, 5]

    # 设置纵坐标、横坐标的刻度
    plt.axis([2, 5, 90, 100])
    # 其中，y的元素分别是2层、3层、4层、5层时的识别准确率
...
# 比较在神经元总数固定的前提下，隐藏层的层数对识别准确率的影响
model_layers_selection()
